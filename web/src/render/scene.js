...
`;
function nullRenderer(kind = "none") {
    return {
        kind,
        setSceneScale: () => { },
        setCentralBody: () => { },
...
const TRAJ_COLOR = [0.18, 1.00, 0.08]; // bright phosphor
// Central body rendered in gray
const BODY_COLOR = [0.50, 0.50, 0.50]; // gray
const BODY_EQ_CLR = [0.60, 0.60, 0.60]; // equator highlight (lighter gray)
const AXIS_COLORS = [
...
function stripToQuads(pos, cols, defCol) {
...
function buildSphere(r, parallels = 18, meridians = 36, ringSegs = 72 * 2, meridianSegs = 36 * 2) {
    const verts = [];
...
function buildSolidSphere(radius, rings = 32, segs = 32) {
    const verts = [];
...
export async function initRenderer(canvas) {
    ...
    const mkThickPipeline = (fsEntry, compare) => device.createRenderPipeline({
        layout: "auto",
    ...
    const mkBG = (p) => device.createBindGroup({
    ...
    let apsesBuf = null;
    let apsesCount = 0;
    let lagrangeBuf = null;
    let lagrangeCount = 0;
    let manifoldBuf = null;
    let manifoldCount = 0;
    let axesBuf = null;
    let axesCount = 0;
    let markerBuf = null;
    ...
    function uploadThick(buf, data) {
        ...
        if (data.length > 0)
            device.queue.writeBuffer(newBuf, 0, data);
        return [newBuf, data.length / THICK_STRIDE];
    }
    function uploadRaw(buf, data, countDivisor) {
        buf?.destroy();
        const newBuf = device.createBuffer({
            size: Math.max(data.byteLength, 4),
            usage: GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_DST,
    ...
    function frame() {
        ...
        if (manifoldBuf && manifoldCount > 0) {
            pass.setPipeline(thickGtPipeline);
            pass.setBindGroup(0, thickGtBG);
            pass.setVertexBuffer(0, manifoldBuf);
            pass.draw(manifoldCount);
            pass.setPipeline(thickLePipeline);
            pass.setBindGroup(0, thickLeBG);
            pass.setVertexBuffer(0, manifoldBuf);
            pass.draw(manifoldCount);
    ...
    const MANIFOLD_COLOR_DEFAULT = [0.65, 0.30, 1.00]; // violet
    function setManifoldTubes(tubes, color) {
        if (tubes.length === 0) {
            manifoldBuf?.destroy();
            manifoldBuf = null;
            manifoldCount = 0;
    ...
    const SEC_BODY_COLOR = [0.55, 0.55, 0.55]; // moon-grey
    function setSecondaryBody(position, radius, color) {
    ...
    const MKR_R = [1.00, 0.35, 0.20]; // radial     — red/orange
    const MKR_I = [0.20, 1.00, 0.30]; // in-track   — green
