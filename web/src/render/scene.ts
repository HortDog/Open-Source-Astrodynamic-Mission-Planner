...
export async function initRenderer(canvas: HTMLCanvasElement): Promise<Renderer> {
  ...
  let futureCount     = 0;
  let coastBuf:       GPUBuffer | null = null;
  let coastCount      = 0;
  ...
  function frame(): void {
    ...
    if (coastBuf && coastCount > 0) {
      pass.setPipeline(thickGtPipeline);
      pass.setBindGroup(0, thickGtBG);
      pass.setVertexBuffer(0, coastBuf); pass.draw(coastCount);
      pass.setPipeline(thickLePipeline);
      pass.setBindGroup(0, thickLeBG);
      pass.setVertexBuffer(0, coastBuf); pass.draw(coastCount);
    }
  ...
  let cachedMarkerPos:   [number, number, number] | null      = null;
  let cachedMarkerVel:   [number, number, number] | null | undefined = undefined;
  let cachedMarkerColor: [number, number, number] | undefined = undefined;
  let cachedApses:    ReadonlyArray<{ position: [number, number, number]; isPeriapsis: boolean }> = [];
  let cachedLagrange: ReadonlyArray<{ position: [number, number, number]; label: string }>        = [];
  ...
  function rebuildAllForZoom(): void {
    rebuildAxes();
